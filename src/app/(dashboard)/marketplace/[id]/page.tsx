...
import { ArrowLeft, Heart, MapPin, Shield, MessageCircle, Share2, Trash2 } from 'lucide-react'
...
export default function ListingDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleContact = () => {
    ...
    window.location.href = '/chat?user=' + listing.user_id
  }
  ...
  if (!listing || listing.error) return (
    <div style={{ textAlign: 'center', padding: '80px 0' }}>
      <p style={{ fontSize: '18px', fontWeight: 700, color: 'var(--on-surface)' }}>Listing not found.</p>
      <Link href="/marketplace" style={{ color: 'var(--primary)', textDecoration: 'none', fontWeight: 600, display: 'block', marginTop: '8px' }}>Back to Marketplace</Link>
    </div>
  )
  ...
  return (
    <div style={{ maxWidth: '900px', margin: '0 auto' }}>
      <Link href="/marketplace" style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', fontSize: '13px', color: 'var(--on-surface-variant)', textDecoration: 'none', marginBottom: '28px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '40px' }}>
          ...
          {images.length > 1 && (
            <div style={{ display: 'flex', gap: '10px', marginTop: '12px' }}>
              {images.map((img: string, i: number) => (
                <button key={i} onClick={() => setSelectedImage(i)} style={{ width: '72px', height: '72px', borderRadius: '12px', overflow: 'hidden', border: `2px solid ${i === selectedImage ? 'var(--primary)' : 'var(--border)'}`, padding: 0, cursor: 'pointer' }}>
        ...
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '16px', gap: '12px' }}>
            <h1 style={{ fontSize: '22px', fontWeight: 800, color: 'var(--on-surface)', lineHeight: 1.3, fontFamily: 'var(--font-manrope)', flex: 1 }}>{listing.title}</h1>
          ...
          <div style={{ fontSize: '36px', fontWeight: 800, marginBottom: '24px', fontFamily: 'var(--font-manrope)' }} className="signature-gradient-text">
            ₹{Number(listing.price).toLocaleString()}
          </div>

          {isOwner ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: '20px' }}>
              {listing.is_available && <button onClick={handleMarkSold} style={{ width: '100%', padding: '14px', borderRadius: '14px', border: '2px solid var(--primary)', background: 'transparent', color: 'var(--primary)', fontWeight: 700, fontSize: '14px', cursor: 'pointer' }}>Mark as Sold</button>}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: '20px' }}>
              <button onClick={handleContact} className="signature-gradient" style={{ width: '100%', padding: '16px', borderRadius: '14px', color: 'white', fontWeight: 700, fontSize: '15px', border: 'none', cursor: 'pointer', boxShadow: '0 8px 25px rgba(53,37,205,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
                <MessageCircle size={18} /> Contact Seller
              </button>
              <button onClick={() => setShowBorrowForm(!showBorrowForm)} style={{ width: '100%', padding: '16px', borderRadius: '14px', fontWeight: 700, fontSize: '15px', cursor: 'pointer', border: '2px solid var(--primary)', background: 'transparent', color: 'var(--primary)' }}>
                {showBorrowForm ? 'Cancel' : 'Request to Borrow'}
              </button>
              {showBorrowForm && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ background: 'var(--surface-container-low)', borderRadius: '16px', padding: '16px', border: '1px solid var(--border)' }}>
                  <p style={{ fontSize: '13px', fontWeight: 700, color: 'var(--on-surface)', marginBottom: '12px' }}>Borrow Details</p>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '10px' }}>
                    <div>
                      ...
                      <input type="date" value={borrowFrom} onChange={e => setBorrowFrom(e.target.value)} style={{ width: '100%', padding: '8px 12px', borderRadius: '10px', border: '1px solid var(--border)', background: 'var(--card)', color: 'var(--on-surface)', fontSize: '13px', outline: 'none' }} />
                    </div>
                    <div>
                      <label style={{ fontSize: '11px', fontWeight: 600, color: 'var(--on-surface-variant)', display: 'block', marginBottom: '4px' }}>Until</label>
                      <input type="date" value={borrowUntil} onChange={e => setBorrowUntil(e.target.value)} style={{ width: '100%', padding: '8px 12px', borderRadius: '10px', border: '1px solid var(--border)', background: 'var(--card)', color: 'var(--on-surface)', fontSize: '13px', outline: 'none' }} />
                    </div>
                  </div>
                  <textarea value={borrowMsg} onChange={e => setBorrowMsg(e.target.value)} placeholder="Why do you need this? (optional)" rows={2} style={{ width: '100%', padding: '8px 12px', borderRadius: '10px', border: '1px solid var(--border)', background: 'var(--card)', color: 'var(--on-surface)', fontSize: '13px', outline: 'none', resize: 'none', marginBottom: '10px' }} />
      ...
      {listing.description && (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} style={{ marginTop: '40px' }}>
          <h2 style={{ fontSize: '20px', fontWeight: 700, color: 'var(--on-surface)', fontFamily: 'var(--font-manrope)', marginBottom: '12px' }}>Description</h2>
