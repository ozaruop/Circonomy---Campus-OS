...
const tabs = ['My Listings', 'My Gigs', 'Reviews Received']
  ...
  return (
    <div style={{ position: 'relative', width: '128px', height: '128px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      ...
      <div style={{ textAlign: 'center', position: 'relative', zIndex: 1 }}>
        ...
        <div style={{ fontSize: '11px', color: 'var(--on-surface-variant)' }}>/ 100</div>
      </div>
    </div>
  )
}
...
export default function ProfilePage() {
  ...
  const trustScore = profileData?.user?.trust_score ?? 50
    ...
    <div style={{ maxWidth: '900px', margin: '0 auto' }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap' }}>
                <span style={{ fontSize: '11px', fontWeight: 700, padding: '4px 12px', borderRadius: '999px', backgroundColor: 'var(--primary-light)', color: 'var(--primary)' }}>
                  {trustScore >= 75 ? 'High Trust Member' : trustScore >= 50 ? 'Member' : 'New Member'}
                </span>
      ...
      {activeTab === 'Reviews Received' && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          {loading ? [...Array(2)].map((_: any, i: number) => <div key={i} style={{ ...card, height: '120px' }} className="animate-pulse" />) :
          myReviews.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '80px 0' }}>
              <div style={{ fontSize: '48px', marginBottom: '16px' }}>⭐</div>
              <p style={{ fontSize: '18px', fontWeight: 700, color: 'var(--on-surface)', fontFamily: 'var(--font-manrope)' }}>No reviews yet</p>
              <p style={{ fontSize: '14px', color: 'var(--on-surface-variant)', marginTop: '8px' }}>Complete transactions to receive reviews from other users.</p>
            </div>
          ) : myReviews.map(review => (
            <div key={review.id} style={{ ...card, padding: '20px' }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: '16px' }}>
                {review.users?.avatar_url ? (
                  <img src={review.users.avatar_url} alt="" style={{ width: '40px', height: '40px', borderRadius: '50%', objectFit: 'cover', flexShrink: 0 }} />
                ) : (
                  <div className="signature-gradient" style={{ width: '40px', height: '40px', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontWeight: 700, fontSize: '15px', flexShrink: 0 }}>
                    {review.users?.full_name?.[0] ?? '?'}
                  </div>
                )}
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '6px' }}>
                    <p style={{ fontWeight: 700, fontSize: '14px', color: 'var(--on-surface)' }}>{review.users?.full_name ?? 'Anonymous'}</p>
                    <span style={{ fontSize: '11px', color: 'var(--outline)' }}>{new Date(review.created_at).toLocaleDateString()}</span>
                  </div>
                  ...
                  <p style={{ fontSize: '13px', color: 'var(--on-surface-variant)', lineHeight: 1.6 }}>{review.comment}</p>
                </div>
              </div>
            </div>
          ))}
