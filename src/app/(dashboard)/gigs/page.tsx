...
import { Clock, Star, Zap, Plus, X } from 'lucide-react'
...
const priceTypes = ['HOURLY','PROJECT','FIXED']
...
export default function GigsPage() {
  ...
  // Count by category for chart
  const demandData = ['Notes','Tutoring','Design','Coding','Other'].map(name => ({
    name, value: gigs.filter(g => g.category === name).length
  }))
  ...
  const handleHire = async () => {
    ...
    if (!payRes.ok) {
      toast.success('Order placed!', { description: 'Razorpay not configured — order saved. Seller will confirm.' })
      setShowHireModal(null); setHireMsg('')
      return
    }
    const payData = await payRes.json()
    const gigTitle = showHireModal?.title ?? 'Gig Payment'
    setShowHireModal(null); setHireMsg('')
    ...
    const rzp = new (window as any).Razorpay({
      ...
      name: 'Circonomy Campus OS',
      description: gigTitle,
      image: '/icon.png',
      theme: { color: '#3525cd' },
      handler: async (response: any) => {
        // 4. Verify signature server-side (MANDATORY)
        ...
        if (verRes.ok) {
          toast.success('Payment successful! 🎉', { description: 'Seller has been notified.' })
          fetchGigs()
        } else {
      ...
      modal: {
        ondismiss: () => {
          toast('Payment cancelled', { description: 'Order saved — pay anytime from Activity tab.' })
        }
      }
    })
    rzp.open()
  ...
  return (
    <div>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
        style={{ ...card, padding: '24px', marginBottom: '28px', overflow: 'visible' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px' }}>
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '4px' }}>
              <Zap size={14} style={{ color: 'var(--primary)' }} />
              <p style={{ fontSize: '11px', color: 'var(--on-surface-variant)', textTransform: 'uppercase', letterSpacing: '0.08em', fontWeight: 600 }}>Live Campus Data</p>
            </div>
            <h3 style={{ fontSize: '18px', fontWeight: 700, color: 'var(--on-surface)', fontFamily: 'var(--font-manrope)' }}>Gig Supply by Category</h3>
            <p style={{ fontSize: '12px', color: 'var(--on-surface-variant)' }}>{gigs.length} active gigs on campus</p>
          </div>
          <div className="signature-gradient" style={{ padding: '8px 16px', borderRadius: '999px', color: 'white', fontSize: '12px', fontWeight: 700 }}>Live</div>
        </div>
        <ResponsiveContainer width="100%" height={120}>
          <BarChart data={demandData} barSize={36}>
            ...
            <YAxis hide />
            <Tooltip contentStyle={{ borderRadius: '12px', border: 'none', backgroundColor: 'var(--card)', color: 'var(--on-surface)' }} cursor={{ fill: 'var(--primary-light)' }} />
            <Bar dataKey="value" fill="#3525cd" radius={[6,6,0,0]} />
          </BarChart>
        </ResponsiveContainer>
      </motion.div>
      ...
      {loading ? (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(280px,1fr))', gap: '20px' }}>
          {[...Array(6)].map((_: any, i: number) => <div key={i} style={{ ...card, height: '340px' }} className="animate-pulse" />)}
        </div>
      ) : filtered.length === 0 ? (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(280px,1fr))', gap: '20px' }}>
          {filtered.map((gig: any, idx: number) => {
                ...
                <div style={{ padding: '16px' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingTop: '12px', borderTop: '1px solid var(--border)', marginBottom: '12px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      ...
                      <span style={{ fontSize: '10px', fontWeight: 700, padding: '3px 8px', borderRadius: '999px', backgroundColor: pt.bg, color: pt.color }}>{gig.price_type ?? 'PROJECT'}</span>
                    </div>
                  ...
                  {isOwner ? (
                    ...
                    <button onClick={() => { setShowHireModal(gig); setHireMsg('') }} className="signature-gradient"
                      style={{ width: '100%', padding: '12px', borderRadius: '12px', fontSize: '13px', fontWeight: 700, border: 'none', cursor: 'pointer', color: 'white', boxShadow: '0 4px 15px rgba(53,37,205,0.25)' }}>
                      Hire Now
                    </button>
                  )}
                </div>
            ...
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }}
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '24px' }}>
                ...
                <div>
                  ...
                  <input value={newDelivery} onChange={e => setNewDelivery(e.target.value)} placeholder="e.g. 3 days, Flexible" style={inputStyle} />
                </div>
              </div>
        ...
        {showHireModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }}
              ...
              <button onClick={handleHire} disabled={hiring} className="signature-gradient"
                style={{ width: '100%', padding: '16px', borderRadius: '14px', color: 'white', fontWeight: 700, fontSize: '15px', border: 'none', cursor: hiring ? 'not-allowed' : 'pointer', opacity: hiring ? 0.7 : 1, boxShadow: '0 8px 25px rgba(53,37,205,0.3)' }}>
