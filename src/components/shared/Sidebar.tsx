    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: collapsed ? 'center' : 'space-between', padding: '0 12px', height: '64px', borderBottom: '1px solid var(--border)', flexShrink: 0 }}>
        ...
        {isMobile ? (
          ...
          <button onClick={toggleCollapsed} title={collapsed ? 'Expand' : 'Collapse'} style={{ width: '28px', height: '28px', borderRadius: '8px', border: '1px solid var(--border)', backgroundColor: 'var(--surface-container)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: 'var(--on-surface-variant)', flexShrink: 0, transition: 'background 0.2s' }}
            onMouseEnter={(e: any) => ((e.currentTarget as HTMLElement).style.backgroundColor = 'var(--primary-light)')}
            onMouseLeave={(e: any) => ((e.currentTarget as HTMLElement).style.backgroundColor = 'var(--surface-container)')}>
      ...
      <nav style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '2px', padding: '4px 8px', overflowY: 'auto' }}>
        {navItems.map(({ href, label, icon: Icon }) => {
          ...
          const showBadge = href === '/chat' && unreadMsgs > 0
          return (
            <Link
              key={href}
              href={href}
              ...
              onMouseEnter={e => { if (!active) (e.currentTarget as HTMLElement).style.backgroundColor = 'var(--surface-container)' }}
            ...
            >
              ...
              <div style={{ position: 'relative', flexShrink: 0 }}>
                <Icon size={17} />
                {showBadge && collapsed && (
                  <span style={{ position: 'absolute', top: -4, right: -4, width: 14, height: 14, borderRadius: '50%', backgroundColor: 'var(--primary)', color: 'white', fontSize: 8, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', border: '2px solid var(--surface)' }}>
              ...
              {!collapsed && showBadge && (
                <span style={{ backgroundColor: 'var(--primary)', color: 'white', fontSize: '10px', fontWeight: 700, padding: '2px 6px', borderRadius: 999, minWidth: 18, textAlign: 'center' }}>
                  {unreadMsgs > 9 ? '9+' : unreadMsgs}
                </span>
              )}
            </Link>
